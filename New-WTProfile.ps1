function New-WTProfile
{
    ...
    When useAcrylic is set to true, it sets the transparency of the window for the profile.
     Accepts floating point values from 0-1 (default 0.
    5).
    #>
    ...
    # If set, will use the default values derived from the schema.
    # Otherwise, only parameters directly provided
    [Alias('Defaults')]
    [switch]
    $Default
    )
    process {
        ...
        foreach ($p in $myCmdMetaData.Parameters.Keys) {
            if ($p -eq 'Default') { continue }
            $k = $p.Substring(0,1).ToLower() + $p.Substring(1)
            $newProfile[$k] =
                if ($Default) {
                    $ExecutionContext.SessionState.PSVariable.Get($p).Value
                } else {
                    $PSBoundParameters[$p]
                }
